...
function formatDetail(detail: unknown): string {
  if (typeof detail === 'object' && detail !== null && !Array.isArray(detail)) {
    return Object.entries(detail as Record<string, string>)
      .map(([k, v]) => `${k}:${v}`)
      .join(' / ');
  }
...
export default function EditItemModal({ item, onClose }: Props) {
  ...
  const [detail, setDetail] = useState(() => formatDetail(item.metadata.detail));
  ...
  return (
    ...
    >
      <div
        className="bg-white rounded-2xl shadow-xl w-full max-w-md p-6 flex flex-col gap-5 max-h-[90vh] overflow-y-auto"
        onClick={(e) => e.stopPropagation()}
      >
        <div className="flex items-center justify-between">
          <h2 className="font-bold text-gray-800 text-lg">아이템 수정</h2>
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          ...
          {extraFields.length > 0 && (
            <div className="flex flex-col gap-2">
              <p className="text-xs font-semibold text-gray-400">추가 속성</p>
